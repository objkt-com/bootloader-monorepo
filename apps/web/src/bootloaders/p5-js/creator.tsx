...
import { createP5JsGenerator } from "@/services/tezos";
import {
...
interface P5JsCreatorProps {
  bootloader: Bootloader;
  className?: string;
}

export function P5JsCreator({ className }: P5JsCreatorProps) {
  ...
  const primarySaleFeePercent = getPrimarySaleFeePercent("p5-js");
  ...
  useEffect(() => {
    setHeader({
      ...
      onBack: step === "publish" ? () => setStep("edit") : null,
    });
  ...
  const handlePublish = async () => {
    if (!tezos || !authToken || !user?.id || !name.trim()) {
      return;
    }
    ...
    try {
      ...
      if (selectedThumbnail && renderJobs[selectedThumbnail]?.seed) {
        metadata.thumbnail_seed = renderJobs[selectedThumbnail].seed;
      } else if (seed) {
        metadata.thumbnail_seed = seed;
      }

      const metadataCid = await uploadGenericWebMetadataJson(authToken, metadata);
      const result = await createP5JsGenerator(
        tezos,
        name.trim(),
        artifactCid,
        metadataCid,
      ...
      if (result.success && result.generatorId) {
        try {
          ...
          console.warn("Failed to store p5-js metadata in D1:", dbError);
        }

        navigate(`/generator/p5-js/${result.generatorId}?pendingCreate=1`, {
          state: {
            pendingCreate: true,
            createdAt: Date.now(),
          },
        ...
        setError(result.error || "Failed to create p5-js generator");
      }
    } catch (publishError) {
      setError(
  ...
  const handleOpenPublishStep = async () => {
    if (!isConnected) {
      await connect();
      return;
    }
    ...
    if (!renderCid || renderSessionCode !== code) {
      void ensureRenderSession().catch((publishSetupError) => {
        setError(
          publishSetupError instanceof Error
            ? publishSetupError.message
            : "Failed to prepare publish step"
        );
      });
    }
  };
      ...
      <div className="h-full flex flex-col">
        <div className="h-11 border-b px-3 flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            p5.js IPFS artifact • Primary fee {primarySaleFeePercent}%
          </span>
        </div>
      ...
      <div className="h-full flex flex-col">
        <div className="h-11 border-b px-3 flex items-center gap-2">
          ...
          >
            ...
            {!isConnected ? "Connect to Publish" : "Publish"}
        ...
        <ScrollArea className="h-full">
          <div className="p-4 space-y-4">
            <div className="border rounded-md p-4 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Shown on the generator page."
                  rows={5}
                />
              </div>
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Included in token metadata generated by the indexer."
                  rows={5}
                />
              </div>
            </div>
            ...
            <div className="border rounded-md p-4 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                When everything looks right, publish the generator and use the selected cover image.
              </p>
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <Button
                  ...
                  disabled={!name.trim() || isPublishing || isConnecting || !renderCid}
                >
    ...
    <div className={cn("h-full", className)}>
      <PanelGroup direction="horizontal">
        <Panel defaultSize={step === "edit" ? 48 : 55} minSize={28}>
          {step === "edit" ? editStep.props.children[0] : publishStep.props.children[0]}
        </Panel>
        ...
        <Panel defaultSize={step === "edit" ? 52 : 45} minSize={28}>
          {step === "edit" ? editStep.props.children[1] : publishStep.props.children[1]}
        </Panel>
      </PanelGroup>
      <P5JsInfoDialog
        open={isInfoDialogOpen}
