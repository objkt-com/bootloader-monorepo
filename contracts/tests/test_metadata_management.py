...
def test_basic_metadata_setting():
    ...
    """
    scenario = sp.test_scenario("Basic Metadata Setting", [svg_js, randomiser])
    ...
    metadata_updates = {
        ...
        "description": sp.bytes(
            "0x47656e657261746976652041727420506c6174666f726d"
        ),  # "Generative Art Platform"
        "version": sp.bytes("0x76312e302e30"),  # "v1.0.0"
        "author": sp.bytes("0x4f626a6b74204c616273"),  # "Objkt Labs"
    }

    contract.set_metadata(metadata_updates, _sender=admin)

    scenario.h2("Metadata is stored correctly")
    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x426f6f746c6f61646572204e4654")
    )
    scenario.verify(
        contract.data.metadata["description"]
        == sp.bytes("0x47656e657261746976652041727420506c6174666f726d")
    )
    scenario.verify(contract.data.metadata["version"] == sp.bytes("0x76312e302e30"))
    scenario.verify(
        contract.data.metadata["author"] == sp.bytes("0x4f626a6b74204c616273")
    )
...
def test_metadata_updates():
    ...
    """
    scenario = sp.test_scenario("Metadata Updates", [svg_js, randomiser])
    ...
    contract.set_metadata(initial_metadata, _sender=admin)

    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x426f6f746c6f61646572")
    )
    ...
    update_metadata = {
        ...
        "description": sp.bytes(
            "0x4e465420506c6174666f726d"
        ),  # "NFT Platform" - new key
    }

    contract.set_metadata(update_metadata, _sender=admin)
    ...
    scenario.verify(
        contract.data.metadata["version"] == sp.bytes("0x76312e302e30")
    )  # updated
    scenario.verify(
        contract.data.metadata["description"] == sp.bytes("0x4e465420506c6174666f726d")
    )  # new
...
def test_multiple_metadata_updates():
    ...
    """
    scenario = sp.test_scenario("Multiple Metadata Updates", [svg_js, randomiser])
    ...
    contract.set_metadata({"name": sp.bytes("0x426f6f746c6f61646572")}, _sender=admin)
    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x426f6f746c6f61646572")
    )

    scenario.h2("Second update")
    contract.set_metadata({"version": sp.bytes("0x76312e302e30")}, _sender=admin)
    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x426f6f746c6f61646572")
    )
    scenario.verify(contract.data.metadata["version"] == sp.bytes("0x76312e302e30"))
    ...
    contract.set_metadata({"author": sp.bytes("0x4f626a6b74204c616273")}, _sender=admin)
    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x426f6f746c6f61646572")
    )
    scenario.verify(contract.data.metadata["version"] == sp.bytes("0x76312e302e30"))
    ...
    contract.set_metadata(
        {"name": sp.bytes("0x426f6f746c6f61646572204e4654")}, _sender=admin
    )
    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x426f6f746c6f61646572204e4654")
    )
...
def test_empty_metadata_updates():
    ...
    - Empty bytes values
    """
    scenario = sp.test_scenario("Empty Metadata Updates", [svg_js, randomiser])
    ...
    contract.set_metadata({}, _sender=admin)
...
def test_large_metadata_values():
    ...
    """
    scenario = sp.test_scenario("Large Metadata Values", [svg_js, randomiser])
    ...
    large_json = sp.bytes(
        "0x7b226e616d65223a22426f6f746c6f61646572222c226465736372697074696f6e223a2247656e657261746976652041727420506c6174666f726d222c2276657273696f6e223a2276312e302e30222c22617574686f72223a224f626a6b74204c616273227d"
    )  # JSON metadata
    ...
    contract.set_metadata(large_metadata, _sender=admin)
    ...
    scenario.verify(
        contract.data.metadata["interfaces"] == sp.bytes("0x5b2254455a4f532d464132225d")
    )
...
def test_metadata_overwrite():
    ...
    """
    scenario = sp.test_scenario("Metadata Overwrite", [svg_js, randomiser])
    ...
    initial_metadata = {
        ...
        "description": sp.bytes(
            "0x4f6c64204465736372697074696f6e"
        ),  # "Old Description"
    }
    ...
    contract.set_metadata(overwrite_metadata, _sender=admin)

    scenario.h2("Updated keys are changed, others remain")
    scenario.verify(
        contract.data.metadata["name"] == sp.bytes("0x4e657720426f6f746c6f61646572")
    )  # updated
    scenario.verify(
        contract.data.metadata["version"] == sp.bytes("0x76322e302e30")
    )  # updated
    scenario.verify(
        contract.data.metadata["author"] == sp.bytes("0x4f6c6420417574686f72")
    )  # unchanged
    scenario.verify(
        contract.data.metadata["description"]
        == sp.bytes("0x4f6c64204465736372697074696f6e")
    )  # unchanged
...
def test_metadata_with_special_characters():
    ...
    scenario = sp.test_scenario(
        "Metadata with Special Characters", [svg_js, randomiser]
    )
    ...
    special_metadata = {
        # JSON structure
        "json": sp.bytes(
            "0x7b226e616d65223a22426f6f746c6f61646572222c2273796d626f6c223a22f09f9a80227d"
        ),  # {"name":"Bootloader","symbol":"🚀"}
        # URL
        "homepage": sp.bytes(
            "0x68747470733a2f2f626f6f746c6f616465722e6172742f"
        ),  # "https://bootloader.art/"
        # Special symbols
        "symbols": sp.bytes("0xe29c85e29c93e29c97"),  # "✅✓✗"
        # Numbers and mixed content
        "mixed": sp.bytes(
            "0x56657273696f6e20312e302e302028323032342d30312d303129"
        ),  # "Version 1.0.0 (2024-01-01)"
    }

    contract.set_metadata(special_metadata, _sender=admin)

    scenario.h2("Special character metadata is stored correctly")
    scenario.verify(
        contract.data.metadata["json"]
        == sp.bytes(
            "0x7b226e616d65223a22426f6f746c6f61646572222c2273796d626f6c223a22f09f9a80227d"
        )
    )
    scenario.verify(
        contract.data.metadata["homepage"]
        == sp.bytes("0x68747470733a2f2f626f6f746c6f616465722e6172742f")
    )
    scenario.verify(
        contract.data.metadata["symbols"] == sp.bytes("0xe29c85e29c93e29c97")
    )
    scenario.verify(
        contract.data.metadata["mixed"]
        == sp.bytes("0x56657273696f6e20312e302e302028323032342d30312d303129")
    )
...
def test_metadata_access_control():
    ...
    """
    scenario = sp.test_scenario("Metadata Access Control", [svg_js, randomiser])
    ...
    scenario.h2("Admin can set metadata")
    contract.set_metadata({"admin_field": sp.bytes("0x61646d696e")}, _sender=admin)
    ...
    contract.set_metadata({"admin_field2": sp.bytes("0x61646d696e32")}, _sender=admin)
    scenario.verify(
        contract.data.metadata["admin_field2"] == sp.bytes("0x61646d696e32")
    )
