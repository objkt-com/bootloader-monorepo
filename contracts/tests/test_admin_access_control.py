...
def test_admin_only_operations():
    ...
    - Adding bootloaders
    """
    scenario = sp.test_scenario("Admin Only Operations", [svg_js, randomiser])
    ...
    contract.add_moderator(
        alice.address, _sender=alice, _valid=False, _exception="ONLY_ADMIN"
    )
    ...
    contract.add_moderator(
        moderator.address, _sender=admin, _valid=False, _exception="ONLY_ADMIN"
    )
...
def test_moderator_permissions():
    ...
    - Flagging generators
    """
    scenario = sp.test_scenario("Moderator Permissions", [svg_js, randomiser])
...
def test_access_control_enforcement():
    ...
    """
    scenario = sp.test_scenario("Access Control Enforcement", [svg_js, randomiser])
    ...
    contract.set_treasury(
        alice.address, _sender=alice, _valid=False, _exception="ONLY_MODS"
    )

    scenario.h2("Non-mods cannot set platform fees")
    contract.set_platform_fee_bps(
        3000, _sender=alice, _valid=False, _exception="ONLY_MODS"
    )
...
def test_platform_fee_limits():
    ...
    """
    scenario = sp.test_scenario("Platform Fee Limits", [svg_js, randomiser])
    ...
    contract.set_platform_fee_bps(
        10001, _sender=admin, _valid=False, _exception="BPS_TOO_HIGH"
    )
