...
class TzKTService {
  ...
  async getGenerator(generatorId) {
    try {
      ...
      const keyData = await this.getBigMapKey(generatorsBigMap.ptr, generatorId.toString());
      if (!keyData) {
        return null;
      }
  ...
  async getFragment(index) {
    try {
      ...
      const keyData = await this.getBigMapKey(fragsBigMap.ptr, index.toString());
      if (!keyData) {
        return null;
      }
  ...
  async getHistoricalBigMapKey(bigmapId, level, key) {
    const url = `${this.baseUrl}/v1/bigmaps/${bigmapId}/historical_keys/${level}/${encodeURIComponent(key)}`;
    try {
      return await this.fetchJson(url);
    } catch (error) {
      if (error.message.includes('404')) {
  ...
  async getGeneratorMints(generatorId, limit = 10) {
    try {
      ...
      const generatorMappingBigMap = await this.getBigMapByPath('generator_mapping');
      ...
      for (const tokenMapping of generatorTokens) {
        ...
        try {
          const tokenMetadata = await this.getBigMapKey(tokenMetadataBigMap.ptr, tokenId.toString());
          const tokenOwner = await this.getBigMapKey(ledgerBigMap.ptr, tokenId.toString());
  ...
  async getOwnedTokens(ownerAddress, limit = 50) {
    try {
      ...
      if (!tokenMetadataBigMap || !generatorMappingBigMap || !generatorsBigMap) {
        console.warn('Required bigmaps not found');
        return [];
      }
      ...
      for (const tokenOwnership of ownedTokens) {
        ...
        try {
          ...
          const generatorMapping = await this.getBigMapKey(generatorMappingBigMap.ptr, tokenId.toString());
          
          if (tokenMetadata && generatorMapping) {
            const generatorId = parseInt(generatorMapping.value);
            
            // Get generator info
            const generatorData = await this.getBigMapKey(generatorsBigMap.ptr, generatorId.toString());
            ...
            if (generatorData) {
              ...
              tokens.push({
                ...
                generatorCode: this.bytesToString(generatorData.value.code),
                owner: ownerAddress
              });
            }
          }
